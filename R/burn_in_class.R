...
#' 
#' @param brck a raster brick as generated by create_brick
#' @param class_name Name of the existing classification onto which to burn the new classes - must be a layer in brck
#' @param output_name name to give the output layer added to brck
#' @param burns list  Named list of layer in brck to burn into discretisation of HRUs that will be stamped onto the classification. Overrides any classification already defined
#' @param json_path Path of folder into which to write the json file identifying which burns were used.
#' 
#' @return a raster layer as brck but with additional layer added.
#'
#' @details Create an additional layer in brck with a classification based on class_name with the burns overwriting the original class values. Burns are applied in the order theya are input.
#' 
#' @export
burn_in_class <- function(brck,class_name,output_name,burns,json_path=getwd()){
