...
In the case of the catchmentRasterStack the initialisation function takes the DEM
...
```{r, sink_fill}
ctch <- sink_fill(ctch)
raster::plot( is.na(ctch[['filled_dem']]) - is.na(ctch[['dem']]),
             main="Areas dropped due to connectivity")
raster::plot( ctch[['filled_dem']] - ctch[['dem']],
...
raster::plot( ctch[['atanb']])
## plot of order of pixels (1 highest)
raster::plot( ctch[['order']] )
```
...
identified.
```{r burn_in_set}
ctch[['greater_500']] <- raster::reclassify(ctch[['filled_dem']],
                                            matrix(c(0,500,NA,
                                                     500,1000,raster::cellStats(ctch[['atb_split']],'max')+1),2,byrow=TRUE))
```
...
```{r burn_in}
ctch <- burn_in_class(ctch,"atb_split","atb_500_split",list("greater_500"))
raster::plot(ctch[["atb_500_split"]])
```
...
We can then use the dynatop package to check the validity of the model
```{r check_model}
