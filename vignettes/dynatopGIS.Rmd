...
topographic index, which is computeed from the upstream area and gradient.
...
gradient. Ths can be evaluated with
```{r, calc_area}
upslope_area(project_path,max_iter = 2000)
plot( raster::raster(file.path(project_path,'upslope_area.tif')))
```
...
subset of between pixel gradients. For a normal 'hillslope' cell these are the
gradients to downslope pixels. In the case of pixels which contain river
channels, or fall on the edge of the catchment with \code{NA} neighbours it is
the average of the gradients from upslope pixels.
```{r, calc_tanb}
tanb(project_path)
plot( raster::raster(file.path(project_path,'tanb.tif')))
```
